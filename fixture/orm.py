...
class ORMFixture:
    ...
    def convert_contacts_to_model(self, contact):
        def convert(contact):
            return Contact(id=str(contact.id), first_name=contact.firstname, last_name=contact.lastname, mobilephone=contact.mobilephone)
    ...
    @db_session
    def get_not_contacts_in_group(self, group):
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == group.id))[0]
